...
// I2C1 pins are defined in ddx_common.h
#include "ddx_common.h"

uint32_t si5351_ssb_base_p1 = 0;
uint32_t si5351_ssb_base_b = 0;
uint8_t si5351_ssb_pll_addr = SI5351_PLLA_PARAMETERS;
float ssb_phase_to_b_factor = 0;

// DMA I2C: non-blocking I2C writes for the hot phase-update path
// Inspired by pico-i2c-dma but without FreeRTOS dependency
static int si5351_dma_chan = -1;
static uint16_t si5351_dma_cmds[6];
#ifdef USE_PIO_I2C
static bool pio_stop_pending = false;  // Deferred STOP from si5351_write_phase_fast()
#endif

void si5351_dma_init(void) {
  si5351_dma_chan = dma_claim_unused_channel(true);
  dma_channel_config c = dma_channel_get_default_config(si5351_dma_chan);
  channel_config_set_read_increment(&c, true);
  channel_config_set_write_increment(&c, false);
  channel_config_set_transfer_data_size(&c, DMA_SIZE_16);
#ifdef USE_PIO_I2C
  channel_config_set_dreq(&c, pio_get_dreq(i2c_pio_inst, i2c_pio_sm, true));
  dma_channel_set_config(si5351_dma_chan, &c, false);
  dma_channel_set_write_addr(si5351_dma_chan,
                             (io_rw_16 *)&i2c_pio_inst->txf[i2c_pio_sm] + 1, false);
#else
  channel_config_set_dreq(&c, i2c_get_dreq(i2c1, true));
  dma_channel_set_config(si5351_dma_chan, &c, false);
  dma_channel_set_write_addr(si5351_dma_chan, &i2c_get_hw(i2c1)->data_cmd, false);
  // Ensure I2C target address is set for Si5351
  i2c_get_hw(i2c1)->enable = 0;
  i2c_get_hw(i2c1)->tar = i2c_bus_addr;
  i2c_get_hw(i2c1)->enable = 1;
#endif
}

void si5351_dma_wait(void) {
  if (si5351_dma_chan < 0) return;
  dma_channel_wait_for_finish_blocking(si5351_dma_chan);
#ifdef USE_PIO_I2C
  // Wait for PIO TX FIFO to drain (bus idle)
  i2c_pio_inst->fdebug = 1u << (PIO_FDEBUG_TXSTALL_LSB + i2c_pio_sm);
  while (!(i2c_pio_inst->fdebug & (1u << (PIO_FDEBUG_TXSTALL_LSB + i2c_pio_sm)))) {
    tight_loop_contents();
  }
  // Send deferred STOP from si5351_write_phase_fast() to release the bus
  if (pio_stop_pending) {
    pio_i2c_stop(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS);
    pio_stop_pending = false;
  }
#else
  // Wait for I2C bus to finish clocking out remaining bytes
  while (i2c_get_hw(i2c1)->status & I2C_IC_STATUS_ACTIVITY_BITS) {
    tight_loop_contents();
  }
#endif
}

void __not_in_flash_func(si5351_write_phase_fast)(int32_t b_diff) {
  static uint32_t last_p1 = 0;

  // Clamp b to valid range [0, 1048574] to prevent unsigned underflow/overflow
  int32_t b_signed = (int32_t)si5351_ssb_base_b + b_diff;
  if (b_signed < 0) b_signed = 0;
  if (b_signed > 1048574) b_signed = 1048574;
  uint32_t b = (uint32_t)b_signed;

  // P1 = 128 * a + floor(128*b/c) - 512
  // Since 'a' is fixed in si5351_ssb_base_p1, we only add the overflow from 128*b/c
  uint32_t p1 = si5351_ssb_base_p1 + (uint32_t)(((uint64_t)b << 7) / 1048575);
  uint32_t p2 = (uint32_t)(((uint64_t)b << 7) % 1048575);

  uint8_t p1_lsb = (uint8_t)(p1 & 0xFF);

  // Wait for previous DMA transfer to complete (should be instantaneous
  // since >62us have elapsed since the last transfer started)
  if (si5351_dma_chan >= 0) {
    dma_channel_wait_for_finish_blocking(si5351_dma_chan);
  }

#ifdef USE_PIO_I2C
  // Send STOP for previous PIO I2C transaction (deferred to minimize latency)
  if (pio_stop_pending) {
    pio_i2c_stop(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS);
    pio_stop_pending = false;
  }

  // Send START + address via blocking PIO writes (fast at 3.4MHz)
  pio_i2c_start(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS);
  pio_i2c_rx_enable(i2c_pio_inst, i2c_pio_sm, false);
  pio_i2c_put16(i2c_pio_inst, i2c_pio_sm, (i2c_bus_addr << 2) | 1u);

  size_t len;
  if (p1_lsb != (uint8_t)(last_p1 & 0xFF)) {
    si5351_dma_cmds[0] = pio_i2c_prepare_byte(si5351_ssb_pll_addr + 4, false);
    si5351_dma_cmds[1] = pio_i2c_prepare_byte(p1_lsb, false);
    si5351_dma_cmds[2] = pio_i2c_prepare_byte((0xF0 | ((p2 >> 16) & 0x0F)), false);
    si5351_dma_cmds[3] = pio_i2c_prepare_byte(((p2 >> 8) & 0xFF), false);
    si5351_dma_cmds[4] = pio_i2c_prepare_byte((p2 & 0xFF), true);
    len = 5;
    last_p1 = p1;
  } else {
    si5351_dma_cmds[0] = pio_i2c_prepare_byte(si5351_ssb_pll_addr + 5, false);
    si5351_dma_cmds[1] = pio_i2c_prepare_byte((0xF0 | ((p2 >> 16) & 0x0F)), false);
    si5351_dma_cmds[2] = pio_i2c_prepare_byte(((p2 >> 8) & 0xFF), false);
    si5351_dma_cmds[3] = pio_i2c_prepare_byte((p2 & 0xFF), true);
    len = 4;
  }

  if (si5351_dma_chan >= 0) {
    dma_channel_set_read_addr(si5351_dma_chan, si5351_dma_cmds, false);
    dma_channel_set_trans_count(si5351_dma_chan, len, true);
    pio_stop_pending = true;  // STOP sent at start of next call
  } else {
    // Blocking fallback
    for (size_t i = 0; i < len; i++)
      pio_i2c_put_or_err(i2c_pio_inst, i2c_pio_sm, si5351_dma_cmds[i]);
    pio_i2c_stop(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS);
  }

#else  // Hardware I2C path

  size_t len;
  if (p1_lsb != (uint8_t)(last_p1 & 0xFF)) {
    // P1 changed, write all 4 registers (30-33)
    si5351_dma_cmds[0] = si5351_ssb_pll_addr + 4;
    si5351_dma_cmds[1] = p1_lsb;
    ...
    // P1 unchanged, only write P2 registers (31-33)
  ...
  if (si5351_dma_chan >= 0) {
    // Non-blocking DMA path: CPU returns immediately while I2C bus runs
    si5351_dma_cmds[0] |= I2C_IC_DATA_CMD_RESTART_BITS;
    si5351_dma_cmds[len - 1] |= I2C_IC_DATA_CMD_STOP_BITS;
    dma_channel_set_read_addr(si5351_dma_chan, si5351_dma_cmds, false);
    dma_channel_set_trans_count(si5351_dma_chan, len, true);
  } else {
    // Blocking fallback (if DMA not initialized)
    uint8_t msg[6];
    for (size_t i = 0; i < len; i++) msg[i] = (uint8_t)si5351_dma_cmds[i];
    i2c_write_blocking(i2c1, i2c_bus_addr, msg, len, false);
  }
#endif
}
...
void si5351_setup_ssb(uint64_t freq_hz) {
  // Use configurable I2C speed for faster phase updates (*iNNN* CAT command)
  extern volatile uint32_t i2c_baud_khz;
#ifdef USE_PIO_I2C
  // PIO I2C: change clock divider (sys_clk / (8 * baudrate))
  ...
  // Target VCO frequency around 800-850 MHz for best phase noise
  uint32_t ms_div = 850000000 / (uint32_t)freq_hz;
  if (ms_div % 2) ms_div--;  // Must be even
  if (ms_div < 6) ms_div = 6;
  if (ms_div > 254) ms_div = 254;
  ...
  ms_reg.p1 = 128 * ms_div - 512;
  ms_reg.p2 = 0;
  ms_reg.p3 = 1;
  set_ms(SI5351_CLK0, ms_reg, 1, 0, 0);

  si5351_ssb_base_p1 = 128 * a - 512;
  si5351_ssb_base_b = b;
  si5351_ssb_pll_addr = SI5351_PLLA_PARAMETERS;

  // Correct scaling for 16kHz sample rate and dynamic crystal frequency
  // Compute in double to avoid precision loss (numerator exceeds float's 7-digit precision)
  ssb_phase_to_b_factor = (float)(((double)1048575 * (double)ms_div * 16000.0) / (2.0 * M_PI * (double)f_xtal));
...
bool si5351_init(uint8_t i2c_addr, uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr) {
  ...
  // Initialize I2C1 (shared with Si4735)
  init_i2c1();

  // Check for a device on the bus, bail out if it is not there
  uint8_t reg_val = 0;
#ifdef USE_PIO_I2C
  int ret = pio_i2c_read_blocking(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS,
                                  i2c_addr, &reg_val, 1);
#else
  bi_decl(bi_2pins_with_func(I2C1_SDA, I2C1_SCL, GPIO_FUNC_I2C));
  int ret = i2c_read_blocking(i2c1, i2c_addr, &reg_val, 1, false);
#endif
  if (ret < 0)
    return false;
...
#ifdef USE_PIO_I2C
  pio_i2c_write_blocking(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS,
                         i2c_bus_addr, msg, (length + 1));
#else
...
#ifdef USE_PIO_I2C
  // PIO I2C: write register address then read (stop-start, Si5351 tolerates this)
  pio_i2c_write_blocking(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS,
                         i2c_bus_addr, &regAddr, 1);
  pio_i2c_read_blocking(i2c_pio_inst, i2c_pio_sm, PIO_I2C_PIN_HS,
                        i2c_bus_addr, &buf, 1);
#else
