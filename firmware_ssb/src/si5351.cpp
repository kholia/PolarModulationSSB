...
void si5351_dma_init(void) {
  ...
  // Pre-configure DMA channel (only read addr + count change per transfer)
...
void si5351_setup_ssb(uint64_t freq_hz) {
  // Use configurable I2C speed for faster phase updates
  // With 2k pull-ups: 1.5MHz safe, 2MHz possible, try higher with *iNNN* CAT command
  // DMA makes writes non-blocking so CPU is free during the I2C transfer
