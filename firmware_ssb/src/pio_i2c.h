...
// Blocking half-word write to PIO TX FIFO
static inline void pio_i2c_put16(PIO pio, uint sm, uint16_t data) {
    while (pio_sm_is_tx_fifo_full(pio, sm)) {
        if (pio_interrupt_get(pio, sm)) return;
    }
    // Writing to the 16-bit alias of the TX FIFO at offset +2 bytes.
    // This is mirrored to the upper bits of the 32-bit FIFO word.
...
bool pio_i2c_dma_ongoing_any(void);
