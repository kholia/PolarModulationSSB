...
uint32_t Adafruit_NeoPixel::getPixelColor(uint16_t n) const {
  ...
  if (brightfr == NULL) {
    ...
    if (wOffset == rOffset) {  // Is RGB-type device
      po = &opixels[n * 3];
      return ((uint32_t)po[rOffset] << 16) | ((uint32_t)po[gOffset] << 8) | (uint32_t)po[bOffset];
    } else {  // Is RGBW-type device
      po = &opixels[n * 4];
      return ((uint32_t)po[wOffset] << 24) | ((uint32_t)po[rOffset] << 16) | ((uint32_t)po[gOffset] << 8) | (uint32_t)po[bOffset];
    }
  }
}
