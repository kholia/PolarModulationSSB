...
float __not_in_flash_func(speech_process)(float x) {
  ...
  float emph = dc_out + 0.7f * (dc_out - emph_prev);
  emph_prev = dc_out;
  ...
  // Target 0.35 vs clip at 0.4 → signal just touches clip threshold.
  // Gentle ~1dB clipping preserves natural dynamics while still boosting
  // average power. 0.5 was too aggressive — crushed all dynamics and caused
  // crackling from the constant full-scale output saturating the PWM path.
  float target_gain = agc_gain;  // Hold previous if env too low
  if (agc_env > 0.005f)
    target_gain = 0.35f / agc_env;
  if (target_gain > 3.0f) target_gain = 3.0f;    // Max +9.5dB (was +20dB — caused AGC pumping/breathing)
