...
// pio_i2c_put16() is now provided as static inline in pio_i2c.h
...
void pio_i2c_start(PIO pio, uint sm, uint pin_hs) {
    if (pin_hs >= NUM_BANK0_GPIOS) {
        // No external HS circuitry: skip I2C-HS master code negotiation.
        // Simple FM+ start condition for non-HS devices (e.g. Si5351).
        pio_i2c_put_or_err(pio, sm, pio_i2c_prepare_instr(3));
        pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC1_SD0_HS]);  // SDA low while SCL high (START)
        pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC0_SD0_HS]);  // SCL low (prepare for data)
        pio_i2c_put_or_err(pio, sm, pio_encode_mov(pio_isr, pio_null));                // Ensure ISR counter is clear
        return;
    }

    // Full I2C-HS protocol: slow start, master code 00001111, nack, then HS start.
...
void pio_i2c_stop(PIO pio, uint sm, uint pin_hs) {
    if (pin_hs >= NUM_BANK0_GPIOS) {
        // Simple FM+ stop for non-HS devices
        pio_i2c_put_or_err(pio, sm, pio_i2c_prepare_instr(3));
        pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC0_SD0_HS]);  // SDA low, SCL low
        pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC1_SD0_HS]);  // SCL high, SDA still low
        pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC1_SD1_HS]);  // SDA high (STOP)
        return;
    }
    ...
    // At this point, the master (us) should reconnect low speed devices from the bus, and
...
static void pio_i2c_wait_idle(PIO pio, uint sm) {
    ...
    uint32_t timeout = 100000;  // ~100k iterations safety net
...
int pio_i2c_disable_sys_irq(PIO pio, uint sm) {
    ...
    if (irq_is_enabled(pio_irq)) {
        pio_set_irq0_source_enabled(pio, (pio_interrupt_source_t)(pis_interrupt0 + sm), false);
