...
static uint32_t polar_output_rate_hz = 16000;
static uint32_t polar_analysis_rate_hz = 16000;

// Runtime-tunable parameters (set via CAT commands *eNNN*, *jNNN*)
float pm_max_phase_step = 0.8f;
float pm_soft_knee = 0.0002f;
float pm_env_threshold = 0.85f;
float pm_env_ratio = 4.0f;
float pm_env_ceiling = 0.98f;
...
// Close approximation to tanh(x) over the phase limiter's useful range.
// The ESP32-S3 libc tanhf implementation is expensive enough to cap this DSP
// chain near 15.5 ksample/s. This rational form is smooth, odd-symmetric, and
// differs by only a few percent where the limiter operates.
static inline float fast_tanh_f(float x) {
...
// The analytic signal is deliberately bounded to a few units, so libc's
// overflow/underflow-safe hypotf is unnecessary here. Two Newton refinements
// of the classic reciprocal-square-root seed keep the relative magnitude
// error down in the few-ppm range without a software square root.
static inline float fast_magnitude_f(float x, float y) {
  float magnitude_squared = x * x + y * y;
  if (magnitude_squared < 1.0e-20f) return 0.0f;

  uint32_t bits;
  memcpy(&bits, &magnitude_squared, sizeof(bits));
  bits = 0x5f375a86U - (bits >> 1);
  float reciprocal;
  memcpy(&reciprocal, &bits, sizeof(reciprocal));
  const float half = 0.5f * magnitude_squared;
  reciprocal *= 1.5f - half * reciprocal * reciprocal;
  reciprocal *= 1.5f - half * reciprocal * reciprocal;
  return magnitude_squared * reciprocal;
}

// Minimax atan polynomial on [0, 1], reflected into the other octants. Its
// peak angular error is about 12 microradians, small enough to retain the FIR's
// sideband rejection while avoiding the very expensive ESP libc atan2f path.
static inline float fast_atan2_f(float y, float x) {
  const float abs_x = fabsf(x);
  const float abs_y = fabsf(y);
  if (abs_x + abs_y < 1.0e-20f) return 0.0f;

  const bool y_larger = abs_y > abs_x;
  const float ratio = y_larger ? abs_x / abs_y : abs_y / abs_x;
  const float ratio2 = ratio * ratio;
  float angle = ratio *
    (0.9998660f + ratio2 *
      (-0.3302995f + ratio2 *
        (0.1801410f + ratio2 * (-0.0851330f + ratio2 * 0.0208351f))));
  if (y_larger) angle = 0.5f * M_PI - angle;
  if (x < 0.0f) angle = M_PI - angle;
  return y < 0.0f ? -angle : angle;
}

void polar_mod_set_sample_rate(uint32_t sample_rate_hz) {
  polar_mod_set_rates(sample_rate_hz, sample_rate_hz);
}

void polar_mod_set_rates(uint32_t output_rate_hz,
                         uint32_t analysis_rate_hz) {
  polar_output_rate_hz = output_rate_hz == 32000 ? 32000 : 16000;
  polar_analysis_rate_hz = analysis_rate_hz == 32000 ? 32000 : 16000;
  // Preserve the same approximate phase slew per second across profiles.
  pm_max_phase_step = polar_output_rate_hz == 32000 ? 0.4f : 0.8f;
  polar_mod_reset();
}
...
float __not_in_flash_func(filter_bandpass_16k)(float x) {
  float s2;
  if (polar_analysis_rate_hz == 32000) {
    // Highpass 300Hz at 32kHz (2nd order Butterworth)
    float s1 = biquad_f(x, 0.9592f, -1.9183f, 0.9592f, -1.9166f,
                        0.9201f, &delay_bp[0]);
    // Lowpass 3000Hz at 32kHz (2nd order Butterworth)
    s2 = biquad_f(s1, 0.0609f, 0.1217f, 0.0609f, -1.1930f, 0.4364f,
                  &delay_bp[2]);
  } else {
    // Highpass 300Hz at 16kHz (2nd order Butterworth)
    float s1 = biquad_f(x, 0.9186f, -1.8372f, 0.9186f, -1.8347f,
                        0.8408f, &delay_bp[0]);
    // Lowpass 3000Hz at 16kHz (2nd order Butterworth)
    s2 = biquad_f(s1, 0.2452f, 0.4905f, 0.2452f, -0.1369f, 0.1177f,
                  &delay_bp[2]);
  }
  return s2;
}
...
void __not_in_flash_func(hilbert_f)(float sample_in, float *i_out, float *q_out) {
  ...
  float q = 0;
  // The Type-III Hilbert kernel is both sparse and antisymmetric. Pair taps
  // h[k] and h[126-k] so one multiply handles both history samples: 31 MACs
  // instead of 62 (or 127 for the naive FIR).
  for (int k = 2; k <= 62; k += 2) {
    int early_idx = hilbert_idx - k;
    if (early_idx < 0) early_idx += 127;
    int late_idx = hilbert_idx - (126 - k);
    if (late_idx < 0) late_idx += 127;
    q += (hilbert_history[early_idx] - hilbert_history[late_idx]) * h_coeffs[k];
  }
...
static inline float compress_envelope(float amplitude) {
  const float ratio = pm_env_ratio < 1.0f ? 1.0f : pm_env_ratio;
  const float threshold = pm_env_threshold;
  if (ratio <= 1.0f || threshold <= 0.0f) return amplitude;

  // A 10% quadratic knee joins unity gain to the selected compression ratio
  // without the derivative discontinuity of a hard vector clipper.
  const float knee = threshold * 0.10f;
  const float low = threshold - 0.5f * knee;
  const float high = threshold + 0.5f * knee;
  float output;
  if (amplitude <= low) {
    output = amplitude;
  } else if (amplitude >= high) {
    output = threshold + (amplitude - threshold) / ratio;
  } else {
    float delta = amplitude - low;
    output = amplitude + (1.0f / ratio - 1.0f) * delta * delta / (2.0f * knee);
  }
  if (pm_env_ceiling > 0.0f && output > pm_env_ceiling)
    output = pm_env_ceiling;
  return output;
}

void __not_in_flash_func(polar_mod_analyze_f)(float data, float *i_out,
                                              float *q_out) {
  ...
  float amplitude = fast_magnitude_f(x, y);
  if (amplitude > 0.0f) {
    float compressed = compress_envelope(amplitude);
    float gain = compressed / amplitude;
    x *= gain;
    y *= gain;
  }
  *i_out = x;
...
void __not_in_flash_func(polar_mod_encode_iq_f)(float x, float y,
                                                float *ampl_out,
                                                float *phase_diff_out) {
  float ampl = fast_magnitude_f(x, y);
  // A band-limited interpolator can ring slightly above two compressed
  // endpoints. Re-apply only the absolute vector ceiling here; phase remains
  // unchanged because both components would receive the same gain.
  if (pm_env_ceiling > 0.0f && ampl > pm_env_ceiling)
    ampl = pm_env_ceiling;
  float angle = fast_atan2_f(y, x);
  ...
  // Phase is circular. First find the shortest signed arc from the phase we
  // actually transmitted to the new analytic-signal phase, then apply the
  // low-level hold to that arc. Blending the two absolute angles directly can
  // turn a tiny +pi/-pi crossing into an almost-pi impulse.
  float angle_diff = angle - last_angle;
  if (angle_diff > M_PI) angle_diff -= 2.0f * M_PI;
  if (angle_diff < -M_PI) angle_diff += 2.0f * M_PI;
  angle_diff *= knee_blend;
  ...
  angle_diff = max_phase_step * fast_tanh_f(angle_diff / max_phase_step);
...
int __not_in_flash_func(modulation_am_pm_f)(float data, float *ampl_out,
                                            float *phase_diff_out) {
  float i, q;
  polar_mod_analyze_f(data, &i, &q);
